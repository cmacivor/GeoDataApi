...
namespace GeoDataService.Controllers
{
    public class MapServerController : ApiController
    {
        ...
        public MapServerController()
        {
            ...
            //_addressesHttpClient.BaseAddress = new Uri("");
        }
        ...
        public async Task<IHttpActionResult> Get(double x, double y)
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                var serialized = Newtonsoft.Json.JsonConvert.DeserializeObject<GeoData.MapServerApiModel.LocationSummary>(result);

                //map the result to ReturnResult class for council district, etc
                var returnResult = new ReturnResult();
                //Council District = check layerName
                //var parcel = serialized.results.FirstOrDefault(x => x.layerName == "Parcel");
                var parcelLayer = serialized.results.Where(layername => layername.layerName == "Parcel");

                var councilDistrictLayer = serialized.results.FirstOrDefault(council => council.layerName == "Council Districts");

                returnResult.CouncilDistrict = councilDistrictLayer.attributes.Name;

                var congressionalDistrictLayer = serialized.results.FirstOrDefault(council => council.layerName == "Federal Congressional Districts");

                returnResult.CongressionalDistrict = congressionalDistrictLayer.attributes.Name;

                var stateHouseDistrictLayer = serialized.results.FirstOrDefault(district => district.layerName == "State House Districts");

                returnResult.HouseDistrict = stateHouseDistrictLayer.attributes.Name;

                var senateDistrictLayer = serialized.results.FirstOrDefault(senate => senate.layerName == "State Senate Districts");

                returnResult.SenateDistrict = senateDistrictLayer.attributes.Name;

                var voterPrecinctLayer = serialized.results.FirstOrDefault(precinct => precinct.layerName == "Voter Precincts");

                
                returnResult.VoterPrecinctNumber = voterPrecinctLayer.attributes.Name;

                //var buildingNumber = serialized.results.FirstOrDefault(buildingNmbr => buildingNmbr.layerName )
